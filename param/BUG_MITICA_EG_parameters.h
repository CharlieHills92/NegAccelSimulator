...
// Simulation OUTPUT 
string output_name="BUG_HNB_BMITICA_OK";  // Name (of the folder containing the results);
...
//parametri 
int numero_cicli = 16;                                 //iteration number
double h = 0.0005;                                   //Mesh cell size [m]

int b_ison = 1;                                       //=1 if B is used
//const std::string bfieldfn = "./BFields/BUG_NITS2017_1x1.txt";  // name of B field map
const std::string bfieldfn = "./BFields/BUG_MITICA_1x1.txt"; 
...
double equip2plot[5]={3, 5, 10, 100, 1000};			// Values of equipotential to be plotted
...
double z_diagn=0.001; // were output (divergence, etc.. ) is evaluated
...
//mesh 
double mesh_x = 0.02;                                 //Mesh x_dimension [m]
double mesh_y = 0.02;                               //Mesh y_dimension [m]
...
/*
const int nscan=4;// number of simulations 
double current_density_scan[nscan]={150,150,150,150}; // extracted currents, A/m2
double V1_scan[nscan]={5,5,5,5}; // grid 1 voltage (tipically EG)
double V2_scan[nscan]={31,33,35,37};// grid 2 voltage (tipically AG1/GG)
*/
